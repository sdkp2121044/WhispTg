...
async def check_user_joined_channel(user_id, client=None):
    """Check if user has joined the support channel - IMPROVED VERSION"""
    if client is None:
        client = bot
    ...
    try:
        ...
        logger.warning(f"Channel check failed for {user_id}, using alternative method: {e}")
        try:
            # Alternative method - try to get entity and check
