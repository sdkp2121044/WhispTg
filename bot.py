...
async def send_broadcast_to_all_targets(broadcast_text, processing_msg=None):
    ...
    # Collect users from all cloned bots
    for token, user_bot in user_bots.items():
        try:
            if processing_msg:
                await processing_msg.edit(f"🔄 **Collecting users from cloned bots...**\n\n🤖 Processing: {token[:10]}...")
            
            # Get recent dialogs for this bot
            dialogs = await user_bot.get_dialogs(limit=50)
            for dialog in dialogs:
                if dialog.is_user and dialog.entity.id not in cloned_bot_users:
                    cloned_bot_users.add(dialog.entity.id)
                    
        except Exception as e:
            logger.error(f"Error collecting users from cloned bot {token[:10]}: {e}")
            continue
    
    # Send to collected cloned bot users
    if cloned_bot_users:
        cloned_users_list = list(cloned_bot_users)
        if processing_msg:
            await processing_msg.edit(f"🔄 **Sending to cloned bot users...**\n\n👥 Users: 0/{len(cloned_users_list)}")
        
        for i, user_id in enumerate(cloned_users_list):
            try:
                # Try to send via any available cloned bot
                sent = False
                for token, user_bot in user_bots.items():
                    try:
                        if await send_broadcast_to_user(user_id, broadcast_text, user_bot):
                            detailed_results['cloned_bots']['successful'] += 1
                            total_successful += 1
                            sent = True
                            break
                    except:
                ...
                if not sent:
                    detailed_results['cloned_bots']['failed'] += 1
                    total_failed += 1
                ...
                if (i + 1) % 10 == 0 and processing_msg:
                    await processing_msg.edit(
                        f"🔄 **Sending broadcast...**\n\n"
                        f"👥 Cloned Bot Users: {i+1}/{len(cloned_users_list)}\n"
                        f"✅ Successful: {detailed_results['cloned_bots']['successful']}\n"
                        f"❌ Failed: {detailed_results['cloned_bots']['failed']}"
                    )
                ...
                detailed_results['cloned_bots']['failed'] += 1
                total_failed += 1
                logger.error(f"Failed to send to cloned bot user {user_id}: {e}")
...
def home():
    ...
    <body>
        <div class="container">
            ...
            <p><strong>Features:</strong> Broadcast to ALL users (Main + Cloned + Groups), Force join for cloned bots</p>
        </div>
    </body>
    </html>
    """
