...
async def precheck_channels(client):
    """Auto-join support channels for cloned bots"""
    targets = ["@shribots", "@idxhelp"]
    for chan in targets:
        try:
            await client(JoinChannelRequest(chan))
            logger.info(f"✅ Auto-joined {chan}")
        except UserNotParticipantError:
            logger.info(f"↻ Already in {chan}")
        except Exception as e:
            logger.warning(f"⚠️ Failed to join {chan}: {e}")
...
async def clone_token_handler(event):
    try:
        ...
        # Auto-join support channels for cloned bot
        ...
        # Setup handlers for cloned bot - WITH FORCE JOIN (EXACTLY LIKE MAIN BOT)
        ...
        async def user_start(event):
            ...
            # Regular users ke liye force join check karo - EXACTLY LIKE MAIN BOT
        ...
        async def user_help_handler(event):
            ...
            # Check force join for regular users - EXACTLY LIKE MAIN BOT
            if event.sender_id != ADMIN_ID:
                has_joined = await check_user_joined_channel(event.sender_id, user_bot)
                if not has_joined:
                    mention = f"[{event.sender.first_name}](tg://user?id={event.sender_id})"
...
**Powered by:** @shribots
                ...
                # Check if user has joined the channel - EXACTLY LIKE MAIN BOT
                has_joined = await check_user_joined_channel(event.sender_id, user_bot)
...
✅ **Auto-Joined Channels:** Enabled
        ...
        await creating_msg.edit(
            ...
            f"**Features Included:**\n"
            f"• 🔒 Force Join Channel (Like Main Bot)\n"
            f"• 📢 Broadcast Support\n"
...
def home():
    ...
    <body>
        <div class="container">
            ...
            <p><strong>Features:</strong> Broadcast to ALL users (Main + Cloned + Groups), Force Join for ALL bots, Auto-Join Channels</p>
        </div>
    </body>
    </html>
    """
