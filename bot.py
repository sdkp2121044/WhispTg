...
async def check_user_joined_channel(user_id, client=None):
    ...
    try:
        await client.get_permissions(SUPPORT_CHANNEL, user_id)
        return True
...
async def broadcast_handler(event):
    ...
    try:
        ...
        await event.reply(
            ...
            buttons=[
                [Button.inline("✅ Yes, Send", f"confirm_broadcast:{hash(broadcast_text)}")],
        ...
        async def user_start(event):
            ...
            if not has_joined:
                ...
                await event.reply(
                    ...
                    f"To use @{bot_me.username}, you must join our official channel:\n"
                    f"@shribots\n\n"
                    f"**Why?**\n"
                    f"• Get latest updates\n"
                    f"• Stay informed about new features\n"
                    f"• Support the developers\n\n"
                    f"After joining, click the refresh button below!",
                    buttons=[
                        [Button.url("📢 Join Channel", f"https://t.me/{SUPPORT_CHANNEL}")],
                        [Button.inline("🔄 Refresh & Check", "check_join_status")]
                    ]
                )
                return
...
⚠️ **Use responsibly!**
            ...
            # In a real implementation, you would store the broadcast messages temporarily
            # For now, we'll use a simple approach - get from recent messages
            original_text = "Broadcast message"  # This should be retrieved from storage
            ...
            for user_id in all_users:
                try:
                    if await send_broadcast_to_user(user_id, original_text):
                        successful += 1
                    else:
                        failed += 1
