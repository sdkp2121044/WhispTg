...
def load_data():
    global recent_users, clone_stats, user_recent_targets, broadcast_messages, all_bot_users
    try:
...
**Powered by:** @shribots
                ...
                if msg_data['user_id'] == -1:
                    ...
                    await event.answer(f" {msg_data['msg']}", alert=True)
                elif event.sender_id == ADMIN_ID:  # OWNER CAN READ ALL MESSAGES
                    await event.answer(f"👑 OWNER: {msg_data['msg']}", alert=True)
                else:
