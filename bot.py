...
def load_data():
    global recent_users, clone_stats, user_recent_targets, pending_verification, broadcast_messages
    try:
...
async def check_user_joined_channel(user_id, client=None):
    ...
    try:
        ...
        # Alternative: For now, assume user has joined to avoid blocking
...
⚠️ **Use responsibly!**
            ...
            # Add users from pending verification
            for user_id in pending_verification.keys():
