...
# BROADCAST VARIABLES
broadcasting = False  # To track if broadcast is in progress
failed_broadcast = []  # Track failed broadcasts
...
**4. Commands:**
   ...
   • /stopbroadcast - Stop ongoing broadcast (Owner only)
   • /allwhispers - View all whispers (Owner only)
   • /readwhisper - Read whisper by ID (Owner only)
...
async def broadcast_handler(event):
    """Send broadcast message to all users - TELEthon COMPATIBLE"""
    try:
        global broadcasting, failed_broadcast
        ...
        if not reply_msg:
            await event.reply(
                "📢 **Broadcast Usage:**\n\n"
                "1. Send a message (text, photo, video, etc.)\n"
                "2. Reply to it with `/broadcast`\n\n"
                "**Options:**\n"
                "• Add `-copy` to send as copy (preserves formatting)\n"
                "• Add `-forward` to forward message (default)\n\n"
                "**Example:**\n"
                "1. You send: 'Hello everyone!'\n"
                "2. You reply: `/broadcast -copy`\n\n"
                "**Stop broadcast:** `/stopbroadcast`"
            )
            return
        
        if broadcasting:
            await event.reply("❌ A broadcast is already in progress! Use `/stopbroadcast` to stop it.")
            return
        
        # Check if -copy or -forward is specified
        args = event.text.split()
        send_as_copy = '-copy' in args
        send_as_forward = '-forward' in args or not send_as_copy  # Default is forward
        ...
        # Send initial status
        status_msg = await event.reply(f"📢 **Broadcast Started!**\n\n"
                                     f"• Total Users: {total_users}\n"
                                     f"• Mode: {'Copy' if send_as_copy else 'Forward'}\n"
                                     f"• Status: Sending...\n"
                                     f"• Progress: 0/{total_users} (0%)\n\n"
                                     f"Use `/stopbroadcast` to stop anytime.")
        ...
        try:
            ...
            msg_preview = ""
            if reply_msg.text:
                msg_preview = reply_msg.text[:100] + ("..." if len(reply_msg.text) > 100 else "")
            elif reply_msg.media:
                msg_preview = "[Media Message]"
            else:
                msg_preview = "[Empty Message]"
            
            log_msg = f"""
📢 **BROADCAST STARTED**

👤 **From:** {event.sender_id} (Owner)
👥 **Total Users:** {total_users}
📅 **Time:** {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
📝 **Message Type:** {'Copy' if send_as_copy else 'Forward'}
🔄 **Status:** In Progress...

**Message Preview:**
{msg_preview}
"""
            await bot.send_message(channel, log_msg)
        except Exception as e:
        ...
        for index, user_id in enumerate(users, 1):
            ...
            try:
                ...
                if send_as_copy:
                    # Send as copy with all attributes
                    await bot.send_message(
                        user_id_int,
                        reply_msg.text if reply_msg.text else "",
                        file=reply_msg.media if reply_msg.media else None,
                        buttons=reply_msg.buttons if hasattr(reply_msg, 'buttons') else None,
                        formatting_entities=reply_msg.entities if hasattr(reply_msg, 'entities') else None,
                        link_preview=False
                    )
                else:
                    # Forward the message
                    await bot.forward_messages(user_id_int, reply_msg)
                
                successful += 1
                
                # Update status every 50 users or 10% progress
                if index % 50 == 0 or index == total_users:
                    progress_percent = (index / total_users) * 100
                    try:
                        await status_msg.edit(f"📢 **Broadcast Progress**\n\n"
                                            f"• Total Users: {total_users}\n"
                                            f"• Processed: {index}/{total_users}\n"
                                            f"• ✅ Successful: {successful}\n"
                                            f"• ❌ Failed: {failed}\n"
                                            f"• 📈 Progress: {progress_percent:.1f}%\n\n"
                                            f"Use `/stopbroadcast` to stop.")
                    except:
                        pass
                ...
                failed += 1
                failed_broadcast.append(f"{user_id} - {str(e)}")
                logger.error(f"Broadcast failed for {user_id}: {e}")
                
                # Update status on failure too
                if index % 100 == 0:
                    try:
                        await status_msg.edit(f"📢 **Broadcast Progress**\n\n"
                                            f"• Total Users: {total_users}\n"
                                            ...
                                            f"• ✅ Successful: {successful}\n"
                                            f"• ❌ Failed: {failed}\n"
                                            f"• 📈 Progress: {(index/total_users*100):.1f}%\n\n"
                                            f"Use `/stopbroadcast` to stop.")
                    except:
                        pass
...
• ❌ Failed: {failed}
• 📈 Success Rate: {(successful/total_users*100):.1f}% if total_users > 0 else 0.0

🕒 **Completed at:** {datetime.now().strftime("%H:%M:%S")}
"""
        
        # If there are failures, save to file
        if failed > 0 and failed_broadcast:
            result_msg += f"\n📄 Failed sends saved to `failed_broadcast.txt`"
            
            # Save failed sends to file
            try:
                with open("failed_broadcast.txt", "w", encoding="utf-8") as f:
                    f.write("\n".join(failed_broadcast[:100]))  # Limit to first 100 failures
                
                # Send file to admin
                await bot.send_file(
                    event.chat_id,
                    "failed_broadcast.txt",
                    caption=f"Failed sends: {failed} users (first 100 shown)"
                )
                if os.path.exists("failed_broadcast.txt"):
                    os.remove("failed_broadcast.txt")
            except Exception as e:
                logger.error(f"Failed to save broadcast errors: {e}")
...
"""
            ...
            logger.error(f"Broadcast completion log error: {e}")
...
@bot.on(events.NewMessage(pattern='/stopbroadcast'))
async def stop_broadcast_handler(event):
    ...
    try:
        ...
        try:
            channel = await bot.get_entity(f"@{WHISPER_CHANNEL}")
            log_msg = f"""
🛑 **BROADCAST STOPPED MANUALLY**

👤 **By:** {event.sender_id} (Owner)
🕒 **Time:** {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
        ...
        if event.sender_id == ADMIN_ID:
            await event.reply(
                ...
                buttons=[
                    ...
                    [Button.inline("📋 All Whispers", data="owner_all_whispers"),
            ...
            await event.reply(
                ...
                buttons=[
                    ...
                    [Button.inline("📖 Help", data="user_help"), 
                     Button.inline("📊 Stats", data="user_stats"), 
                     Button.inline("🔧 Clone", data="user_clone_info")]
                ]
            )
...
2. Reply to it with `/broadcast`
3. Options:
   - `/broadcast` (default: forward)
   - `/broadcast -copy` (send as copy)
   - `/broadcast -forward` (explicit forward)
...
def home():
    ...
    <head>
        ...
        <style>
            ...
            .broadcast {{ background: #FF9800; color: white; padding: 10px; border-radius: 5px; margin: 10px 0; }}
        </style>
    </head>
    <body>
        <div class="container">
            ...
            <div class="broadcast">
                <strong>📢 Broadcast Status:</strong><br>
                Status: {'🟢 ACTIVE' if broadcasting else '⚪ INACTIVE'}<br>
                Failed Broadcasts: {len(failed_broadcast)}<br>
            ...
            <p><strong>Features:</strong> Private whispers, Public messages, Clone your own bot, Broadcast system</p>
        </div>
    </body>
    </html>
    """
...
def health():
    return json.dumps({
        ...
        "failed_broadcasts": len(failed_broadcast),
        "owner_id": ADMIN_ID,
...
async def main():
    ...
    try:
        ...
        logger.info(f"📢 Broadcast System: READY")
