...
**Support:** @shribots
        ...
        for i, group in enumerate(admin_groups):
            try:
                await bot.send_message(group, promotion_text)
                success_count += 1
                logger.info(f"✅ Promotion sent to: {getattr(group, 'title', 'Unknown')}")
                ...
                failed_count += 1
                logger.error(f"❌ Failed to send promotion to {getattr(group, 'title', 'Unknown')}: {e}")
        ...
        await event.reply("❌ Error sending group promotion!")
