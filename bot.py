...
async def check_user_joined_channel(user_id, client=None):
    """Check if user has joined the support channel - SIMPLIFIED VERSION"""
    if client is None:
        client = bot
        
    try:
        # TEMPORARY FIX: Always return True for testing
        # Remove this when bot becomes channel admin
        logger.info(f"Channel check for user {user_id} - TEMPORARILY ALLOWED")
        return True
        
        # UNCOMMENT BELOW CODE WHEN BOT BECOMES CHANNEL ADMIN
        """
        # Get channel entity
        entity = await client.get_entity(SUPPORT_CHANNEL)
        
        # Method 1: Try direct participant check
        try:
        ...
        try:
            # For public channels, check if user can access
            participants = await client.get_participants(entity, limit=50)
            user_ids = [user.id for user in participants if hasattr(user, 'id')]
        ...
        # If all methods fail, assume user has not joined
        return False
        """
        
    except Exception as e:
        logger.error(f"Channel check error for user {user_id}: {e}")
        # In case of error, allow user to continue (better user experience)
        return True
...
async def broadcast_handler(event):
    ...
    try:
        ...
        await event.reply(
            ...
            f"⚠️ This will be sent to {len(all_bot_users)} users. Continue?",
            buttons=[
