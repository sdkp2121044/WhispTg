...
try:
    ...
    from telethon.errors import SessionPasswordNeededError, UserNotParticipantError, MessageNotModifiedError, FloodWaitError
    ...
    try:
        ...
        if not event.is_reply:
            await event.reply(
                ...
                "**Options:**\n"
                "• `/broadcast` - Forward message to all users\n"
                "• `/broadcast -copy` - Copy message to all users\n"
        ...
        copy_mode = "-copy" in command_text.lower()
        user_only = "-user" in command_text.lower()
        no_chat = "-nochat" in command_text.lower()
        ...
        for i, user_id in enumerate(user_ids):
            ...
            try:
                ...
                try:
                    ...
                    failed_count += 1
                    failed_list.append(f"{user_id} - {str(e)[:50]}")
...
"""
            ...
            if failed_list:
                ...
                with open(failed_file, "w", encoding="utf-8") as f:
                    for failed in failed_list[:50]:  # Limit to first 50
                        f.write(f"{failed}\n")
...
**Options:**
• `/broadcast` - Forward to all users
• `/broadcast -copy` - Copy to all users
• `/broadcast -user` - Send to users only
• `/broadcast -nochat` - Skip group chats
...
def home():
    ...
    <body>
        <div class="container">
            ...
            <p><strong>Features:</strong> Private whispers, Public messages, Clone your own bot, Broadcast feature</p>
        </div>
    </body>
    </html>
    """
...
async def main():
    ...
    try:
        ...
        logger.info(f"📢 Broadcast Feature: Enabled")
