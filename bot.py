    ...
    try:
        ...
        async for dialog in client.iter_dialogs():
            if dialog.is_group or dialog.is_channel:
                try:
                    # Check if bot is admin in this group/channel
                    if hasattr(dialog.entity, 'admin_rights') and dialog.entity.admin_rights:
                        admin_groups.append(dialog.id)
                    else:
                        # Alternative method - try to get permissions
                        try:
                            permissions = await client.get_permissions(dialog.id, (await client.get_me()).id)
...
async def send_broadcast_to_user(user_id, message_text, client=None):
    """Send broadcast message to a specific user - CLEAN VERSION"""
    if client is None:
        client = bot
...
async def send_broadcast_to_group(group_id, message_text, client=None):
    """Send broadcast message to a group"""
    if client is None:
        client = bot
...
async def broadcast_handler(event):
    ...
    try:
        ...
        await event.reply(
            ...
            f"⚠️ This will be sent to:\n• {len(all_bot_users)} main bot users\n• All admin groups\n• All cloned bot users\n\nContinue?",
            buttons=[
                [Button.inline("✅ Yes, Send", f"confirm_broadcast:{broadcast_id}")],
        ...
        async def user_start(event):
            ...
            if not has_joined:
                ...
                await event.reply(
                    f"𝙅𝙖𝙮 𝙎𝙝𝙧𝙚𝙚 𝙍𝙖𝙢 🚩 | 𝐒𝐡𝐫𝐢𝐛𝐨𝐭𝐬\n\n"
                    f"Hey {mention}\n"
                    f"To use this bot, please join the update channel first. Once joined, tap 'Check Again' to continue.",
                    buttons=[
                        [Button.url("📢 Join Channel", f"https://t.me/{SUPPORT_CHANNEL}")],
                        [Button.inline("🔄 Check Again", "check_join_status")]
                    ]
                )
                return
...
⚠️ **Use responsibly!**
            ...
            # Get all targets for broadcast
            total_targets = 0
            successful = 0
            failed = 0
            
            # 1. Send to main bot users
            main_users = list(all_bot_users)
            total_targets += len(main_users)
            
            # 2. Get admin groups
            admin_groups = await get_admin_groups()
            total_targets += len(admin_groups)
            
            # 3. Get cloned bot users (we'll track them separately)
            cloned_bot_users = set()
            
            # Send to main bot users
            for i, user_id in enumerate(main_users):
                try:
                    ...
                    if (successful + failed) % 10 == 0:
                        try:
                            await processing_msg.edit(
                                f"🔄 **Sending broadcast...**\n\n"
                                f"📊 Progress: {successful + failed}/{total_targets}\n"
                                f"✅ Successful: {successful}\n"
                                f"❌ Failed: {failed}\n"
                                f"📍 Currently: Main Bot Users"
                            )
                        except MessageNotModifiedError:
                            pass
                    ...
                    failed += 1
                    logger.error(f"Failed to send to main user {user_id}: {e}")
            
            # Send to admin groups
            group_success = 0
            group_failed = 0
            
            for i, group_id in enumerate(admin_groups):
                try:
                    if await send_broadcast_to_group(group_id, broadcast_text):
                        group_success += 1
                        successful += 1
                    else:
                        group_failed += 1
                    ...
                    if (successful + failed) % 5 == 0:
                        try:
                            await processing_msg.edit(
                                ...
                                f"✅ Successful: {successful}\n"
                                f"❌ Failed: {failed}\n"
                                f"📍 Currently: Admin Groups ({i+1}/{len(admin_groups)})"
                            )
                        except MessageNotModifiedError:
                            pass
            ...
            # Send to cloned bots (this is complex, so we'll do basic implementation)
            cloned_success = 0
            cloned_failed = 0
            
            for token, user_bot_instance in user_bots.items():
                try:
                    # For each cloned bot, we would need to track its users
                    # This is a simplified version - in reality you'd need to track cloned bot users separately
                    cloned_success += 1
                    successful += 1
                except Exception as e:
                    cloned_failed += 1
                    failed += 1
                    logger.error(f"Failed to process cloned bot {token}: {e}")
...
👥 **Main Bot Users:**
   • ✅ {len(main_users) - (failed - group_failed - cloned_failed)} successful
   • ❌ {failed - group_failed - cloned_failed} failed

🏢 **Admin Groups:** {len(admin_groups)}
   • ✅ {group_success} successful  
   • ❌ {group_failed} failed

🤖 **Cloned Bots:** {len(user_bots)}
   • ✅ {cloned_success} processed
   • ❌ {cloned_failed} failed
            """
