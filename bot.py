    ...
    try:
        ...
        if not event.reply_to_message_id:
            await event.reply(
                ...
                "• Add `-forward` to forward message\n"
                "• Use `/stopbroadcast` to stop ongoing broadcast"
            )
            return
        ...
        send_as_forward = '-forward' in args
...
**Message Preview:**
{reply_msg.text[:100] if reply_msg.text else '[Media Message]'}
"""
        ...
        for index, user_id in enumerate(users, 1):
            ...
            try:
                if send_as_copy:
                    await bot.send_message(int(user_id), reply_msg.text, 
                                         file=reply_msg.media if reply_msg.media else None,
                                         buttons=reply_msg.buttons if reply_msg.buttons else None,
                                         formatting_entities=reply_msg.entities if reply_msg.entities else None)
                elif send_as_forward:
                    await bot.forward_messages(int(user_id), reply_msg)
                else:
                    # Default: forward
                    await bot.forward_messages(int(user_id), reply_msg)
...
"""
        ...
        if failed > 0:
            ...
            with open("failed_broadcast.txt", "w", encoding="utf-8") as f:
                f.write("\n".join(failed_broadcast))
            
            # Send file to admin
            await bot.send_file(
                event.chat_id,
                "failed_broadcast.txt",
                caption=f"Failed sends: {failed} users"
            )
...
1. Type your message
2. Reply to it with `/broadcast`
3. Add `-copy` to send as copy (with formatting)
4. Add `-forward` to forward message

**Example:**
Your message: "Hello everyone!"
Then reply: `/broadcast -copy`
